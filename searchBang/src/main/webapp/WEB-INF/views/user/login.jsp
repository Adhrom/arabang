...
<script type="text/javascript">
	...
	function statusChangeCallback(response){
		...
		if(response.status === 'connected'){
			...
			document.getElementById('test1').innerHTML = 'please log' + 'into this app';
		}else{
			// fb 로그인 되어있찌 않음, 따라서 앱에 로그인 되어있는지 여부가 불확실
			document.getElementById('test1').innerHTML = 'please log' + 'into facebook';
		}
	}
	...
	function testAPI() {
		FB.api('/me', {fields : 'name,email'},function(response) { 
			...
			document.getElementById('test1').innerHTML = nickname +" : " +email; // 테스트 구문..
			// 여기까지 이메일 & 이름 가져오는건 성공함. 
			$.ajax({
				type : "POST",
				url : "facebookInfo.bang",
				data : {
					"nickname" : nickname,
					"email" : email
				},
